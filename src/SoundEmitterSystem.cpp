...
#if defined( USE_OPENAL ) && defined( CLIENT_DLL )
...
static CSoundEmitterSystem g_SoundEmitterSystem_Base( "CSoundEmitterSystem" );

IGameSystem *SoundEmitterSystem_Base()
{
    return &g_SoundEmitterSystem_Base;
}

class COpenALEmitterSystem : public CBaseGameSystem
{
public:
    COpenALEmitterSystem( char const *name )
    {
    }
    ...
    virtual bool Init() 
    {
        return g_SoundEmitterSystem_Base.Init();
    }
    virtual void Shutdown() 
    {
        g_SoundEmitterSystem_Base.Shutdown();
    }
    ...
    virtual void LevelInitPreEntity()
    {
        g_SoundEmitterSystem_Base.LevelInitPreEntity();
    }

    virtual void LevelInitPostEntity()
    {
        g_SoundEmitterSystem_Base.LevelInitPostEntity();
    }
    virtual void LevelShutdownPostEntity()
    {
        g_SoundEmitterSystem_Base.LevelShutdownPostEntity();
    }

    void InternalPrecacheWaves( int soundIndex ) 
    {
        g_SoundEmitterSystem_Base.InternalPrecacheWaves(soundIndex);
    }

    void InternalPrefetchWaves( int soundIndex ) 
    {
        g_SoundEmitterSystem_Base.InternalPrefetchWaves(soundIndex);
    }

    HSOUNDSCRIPTHANDLE PrecacheScriptSound( const char *soundname ) 
    {
        return g_SoundEmitterSystem_Base.PrecacheScriptSound(soundname);
    }

    void PrefetchScriptSound( const char *soundname ) 
    {
        g_SoundEmitterSystem_Base.PrefetchScriptSound(soundname);
    }
...
public:
    ...
    void EmitSoundByHandle( IRecipientFilter& filter, int entindex, const EmitSound_t & ep, HSOUNDSCRIPTHANDLE& handle )
    {
        ...
        else
        {
            ...
            // Old-School system
            g_SoundEmitterSystem_Base.EmitSoundByHandle(filter, entindex, ep, handle);
        }
#else
        g_SoundEmitterSystem_Base.EmitSoundByHandle(filter, entindex, ep, handle);
#endif
    ...
    void EmitCloseCaption( IRecipientFilter& filter, int entindex, bool fromplayer, char const *token, CUtlVector< Vector >& originlist, float duration, bool warnifmissing /*= false*/ )
    {
        g_SoundEmitterSystem_Base.EmitCloseCaption(filter, entindex, fromplayer, token, originlist, duration, warnifmissing);
    }

    void EmitCloseCaption( IRecipientFilter& filter, int entindex, const CSoundParameters & params, const EmitSound_t & ep )
    {
        g_SoundEmitterSystem_Base.EmitCloseCaption(filter, entindex, params, ep);
    }
    ...
    void EmitAmbientSound( int entindex, const Vector& origin, const char *soundname, float flVolume, int iFlags, int iPitch, float soundtime /*= 0.0f*/, float *duration /*=NULL*/ )
    {
        ...
        g_SoundEmitterSystem_Base.EmitAmbientSound(entindex, origin, soundname, flVolume, iFlags, iPitch, soundtime, duration);
    }

    void StopSoundByHandle( int entindex, const char *soundname, HSOUNDSCRIPTHANDLE& handle )
    {
        g_SoundEmitterSystem_Base.StopSoundByHandle( entindex, soundname, handle);
    }

    void StopSound( int entindex, const char *soundname )
    {
        g_SoundEmitterSystem_Base.StopSound(entindex, soundname);
    }

    void StopSound( int iEntIndex, int iChannel, const char *pSample )
    {
        g_SoundEmitterSystem_Base.StopSound(iEntIndex, iChannel, pSample);
    }
    ...
    void EmitAmbientSound( int entindex, const Vector &origin, const char *pSample, float volume, soundlevel_t soundlevel, int flags, int pitch, float soundtime /*= 0.0f*/, float *duration /*=NULL*/ )
    {
        ...
        g_SoundEmitterSystem_Base.EmitAmbientSound(entindex, origin, pSample, volume, soundlevel, flags, pitch, soundtime, duration);
    }
};
